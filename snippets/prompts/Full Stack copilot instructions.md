...
- domain folder:
  - entities: contains the domain entities such as `User`, `Order`, `Inventor`, `Product`,
    ...
    - these can be enforced either through `pydantic.BaseModel` (if there is no SQL database in the application),  or `SQLAlchemy` ORM (this is an opinionated way I prefer to set my entities because I don't like duplicating the class definition)
      models
  - messages: contains the domain messages such as `UserCreated`, `OrderPlaced`, etc.. for event driven architectures (not common)
...
- `use_cases` folder:
  - contains the use case services for the application such as `create_user`,
    ...
    - the use cases are pure functions that are decoupled from the infrastructure,
      they transform data transfer objects (DTOs) such as `CreateUserRequest` to
      `CreateUserResponse` via an execute method
    - as such the use cases should never implement external dependencies,
      they should only use the domain entities, messages, data transfer objects and concrete adapters
    - Use  interfaces such as ports located in the `use_cases` folder when there is more than one adapter for a particular infrastructure capability i.e. more than one email provider, payment processor or time series database 
